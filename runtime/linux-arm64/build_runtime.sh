...
if command -v apt-get >/dev/null 2>&1; then
  echo "Dependências de build recomendadas (Debian/Ubuntu/Raspberry Pi OS 64):"
  echo "  sudo apt-get install -y build-essential cmake git curl unzip libopenblas-dev libssl-dev"
fi
...
# Não empacotar um venv pronto: ele será criado na máquina destino.
...
echo "O venv Python será criado pelo instalador na máquina de destino."
