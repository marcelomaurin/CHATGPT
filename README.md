...
| openai_voice | Stable / Beta | `SoundFilters`<br>`TAIAudioInput`<br>`TAISpeechRecognizer`<br>`TAIVoiceSynthesizer` (motores nativos e provedores remotos: OpenAI, OpenAI-Compatible, Custom HTTP) | `sound_filters_demo`, `audio_capture_demo`, `sound_filters_visual_demo`, `speech_recognizer_demo`, `voice_synthesizer_complete_demo`, `voicesynthesizer_demo`, `voice_remote_provider_demo` | — |
