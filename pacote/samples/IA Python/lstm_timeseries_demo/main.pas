...
begin
  ...
  try
  FAILSTM.LookbackWindow := 24;
  FAILSTM.ForecastSteps := 5;
  FAILSTM.Epochs := 10;
  
  AddLog('LSTM Timeseries Predictor Properties:');
  AddLog('  LookbackWindow: ' + IntToStr(FAILSTM.LookbackWindow));
  AddLog('  ForecastSteps: ' + IntToStr(FAILSTM.ForecastSteps));
  ...
  begin
    ...
    try
      if FAILSTM.TrainModel(FEditHistory.Text) then
      begin
        FAILSTM.Forecast;
        AddLog('LSTM Forecasting complete.');
      end
      else
