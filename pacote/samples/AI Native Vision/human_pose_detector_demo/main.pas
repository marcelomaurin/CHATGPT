...
begin
  ...
  if not FDetector.RequiredMethodsOK then
  begin
  ...
  try
    ...
    if Success and FDetector.LastResultData.HasPose then
    begin
      lblScore.Caption := Format('Confiança: %.2f%%', [FDetector.LastResultData.Score * 100.0]);
      lblStatus.Caption := 'Estado: Pose Detectada';
      LogMsg(Format('btnDetectClick: Pose detected. Score: %.4f', [FDetector.LastResultData.Score]));
      ...
      begin
        LogMsg(Format('Ponto #%d [%s]: (X: %.4f, Y: %.4f, Z: %.4f, Visibilidade: %.2f, Presença: %.2f)',
          [I,
           FDetector.LastResultData.Landmarks[I].Name,
           FDetector.LastResultData.Landmarks[I].X,
           FDetector.LastResultData.Landmarks[I].Y,
           FDetector.LastResultData.Landmarks[I].Z,
           FDetector.LastResultData.Landmarks[I].Visibility,
           FDetector.LastResultData.Landmarks[I].Presence]));
      end;
      LogMsg('============================');
...
var
  LRect: TRect;
begin
  if Assigned(FDetector) and FDetector.LastResultData.HasPose then
  begin
    ...
    FDetector.DrawOnCanvasRect(pbCanvas.Canvas, LRect);
  end;
end;
