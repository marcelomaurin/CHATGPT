...
type
  ...
  private
    ...
    function ReadLineFromProcess(out ALine: string; AMaxWaitMS: Integer = 5000): Boolean;
...
begin
  ...
  begin
    FDiagnosticLog.Add('DLL já carregada anteriormente.');
    Exit(True);
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        FDiagnosticLog.Add('Sucesso ao carregar DLL: ' + FLoadedDLLPath);
        Result := True;
        Break;
      end;
    end;
    ...
    begin
      case FCompiledPlatform of
        ...
        ppLinuxARM64:
          FLastError := 'Falha ao carregar libpython.' + sLineBreak +
                        ...
                        'Instale a biblioteca Python compatível (aarch64).';
        ppLinuxARM:
          FLastError := 'Falha ao carregar libpython.' + sLineBreak +
      ...
      FLastLoadStep := 'Falha ao carregar DLL';
      Exit;
    end;
  finally
    Candidates.Free;
...
begin
  Result := False;
  FLastLoadStep := 'Resolvendo funções da DLL';
  
  // Resolve required ones
  ...
  // Try optional wrapper / modern API resolution
  ...
  FRequiredMethodsOK := Assigned(Py_Initialize) and 
  ...
  FOptionalMethodsOK := Assigned(PyImport_AddModule) and
  ...
  begin
    FLastError := 'Funções essenciais da API C do Python ausentes na DLL.';
    FDiagnosticLog.Add('Erro: ' + FLastError);
    UnloadPythonDLL;
    Exit;
  end;
...
begin
  Result := False;
  FDiagnosticLog.Add('Validando restrições de versão: ' + IntToStr(FPythonMajor) + '.' + IntToStr(FPythonMinor));
  ...
  begin
    FDiagnosticLog.Add('Aviso: Versão do Python major desconhecida (' + IntToStr(FPythonMajor) + ').');
  end;
  
  if (FPythonMajor = 3) and ((FPythonMinor < FMinPythonMinor) or (FPythonMinor > FMaxPythonMinor)) then
  begin
    FLastError := Format('Versão do Python não suportada: %d.%d. Esperado entre %d.%d e %d.%d',
                         [FPythonMajor, FPythonMinor, FMinPythonMajor, FMinPythonMinor, FMaxPythonMajor, FMaxPythonMinor]);
    FDiagnosticLog.Add('Erro: ' + FLastError);
    Exit;
  end;
...
begin
  ...
  if (S <> '') and (State = 2) then
    FPythonPatch := StrToIntDef(S, 0);
end;
...
begin
  ...
  // 1. Capture version and architecture synchronously
  ...
  try
    ...
    try
      ...
      FProcess.Parameters.Add('import sys; print(''VER:'' + sys.version); print(''ARCH:'' + (''x86_64'' if sys.maxsize > 2**32 else ''i386''))');
      ...
      try
        ...
        begin
          OutputStr := ''; // clear output on error
          Params.Clear;
          Params.LoadFromStream(FProcess.Stderr);
          FLastError := 'Python Stderr: ' + Trim(Params.Text);
          FDiagnosticLog.Add('Erro Stderr: ' + FLastError);
        end;
      except
        on E: Exception do
        begin
          FLastError := 'Falha ao executar executável Python (' + PythonExe + '): ' + E.Message;
          FDiagnosticLog.Add('Erro: ' + FLastError);
          Exit;
        end;
      end;
    ...
    FArchitectureCompatible := (FLazarusArchitecture = FPythonArchitecture);
    FDiagnosticLog.Add('Compatibilidade de arquitetura: ' + BoolToStr(FArchitectureCompatible, 'Yes', 'No'));
  ...
  // 2. Launch persistent background process
  FTempFileName := IncludeTrailingPathDelimiter(GetTempDir) + 'py_conn_' + IntToStr(Random(1000000)) + '.py';
  Params := TStringList.Create;
  try
    ...
    try
      ...
      FDiagnosticLog.Add('Script temporário do processo criado em: ' + FTempFileName);
    except
      on E: Exception do
      begin
        FLastError := 'Falha ao gravar script temporário em ' + FTempFileName + ': ' + E.Message;
        FDiagnosticLog.Add('Erro: ' + FLastError);
        Exit;
      end;
    end;
  ...
  try
    try
      FProcess.Execute;
    FDiagnosticLog.Add('Processo persistente executado com o script temporário. Aguardando PYTHON_CONNECTOR_READY...');
    ...
    begin
      ...
      begin
        FLastError := 'Saída inesperada do processo Python ao iniciar: ' + Line;
        FDiagnosticLog.Add('Erro: ' + FLastError);
        StopPythonProcess;
      end;
    end
    ...
    begin
      ...
      begin
        Params.Clear;
        Params.LoadFromStream(FProcess.Stderr);
        FLastError := 'Erro ao iniciar interpretador Python: ' + Trim(Params.Text);
      end
      else
      begin
        FLastError := 'Time out aguardando prontidão do processo Python. ExitStatus: ' + IntToStr(FProcess.ExitStatus);
      end;
      FDiagnosticLog.Add('Erro: ' + FLastError);
      StopPythonProcess;
    end;
    ...
    on E: Exception do
    begin
      FLastError := 'Exceção ao executar processo persistente Python: ' + E.Message;
      FDiagnosticLog.Add('Erro: ' + FLastError);
      StopPythonProcess;
    end;
  end;
...
begin
  ...
  begin
    try
      ...
      begin
        try
          FProcess.Input.Write(sLineBreak[1], Length(sLineBreak));
        except
        end;
...
begin
  ...
  while (GetTickCount64 - StartTicks < QWord(AMaxWaitMS)) do
  begin
...
begin
  ...
  // 1. Check DLLPath (if it points to an exe/executable file)
  if SameText(ExtractFileExt(FDLLPath), {$IFDEF MSWINDOWS}'.exe'{$ELSE}''{$ENDIF}) and FileExists(FDLLPath) then
    Exit(FDLLPath);
...
begin
  ...
  try
    ...
    begin
      ...
      if (Pos('64 bit', FPythonVersionText) > 0) or (Pos('64-bit', FPythonVersionText) > 0) or 
         (Pos('x64', FPythonVersionText) > 0) or (Pos('AMD64', FPythonVersionText) > 0) then
        FPythonArchitecture := 'x86_64'
      else
        FPythonArchitecture := 'i386';
    ...
    FActive := True;
    Result := True;
    FDiagnosticLog.Add('DLL Python carregada, inicializada e testada com sucesso.');
  except
    on E: Exception do
    begin
      FLastError := 'Exceção ao inicializar DLL do Python: ' + E.Message;
      FDiagnosticLog.Add('Erro: ' + FLastError);
      UnloadPythonDLL;
      FInitialized := False;
      FActive := False;
...
begin
  Result := False;
  FLastLoadStep := 'Executando Autoteste (SelfTest)';
  ...
  begin
    ...
    begin
      FLastError := 'Python executa scripts, mas falhou ao recuperar variáveis.' + sLineBreak +
                    'Verifique assinatura das funções PyImport_AddModule, PyModule_GetDict, ' +
                    'PyDict_GetItemString, PyObject_Str e PyUnicode_AsUTF8.';
    end
    else
    begin
      FLastError := 'Python executa scripts, mas falhou ao recuperar variáveis através do processo.';
    end;
    FDiagnosticLog.Add('Erro: ' + FLastError);
    Exit;
  end;
...
begin
  ...
  case FCompiledPlatform of
    ppWindows64:
    begin
      AReport.Add('Componente compilado para: Windows 64 bits');
      AReport.Add('Serão procuradas apenas DLLs Windows 64 bits.');
    end;
    ppWindows32:
    begin
      AReport.Add('Componente compilado para: Windows 32 bits');
      AReport.Add('Serão procuradas apenas DLLs Windows 32 bits.');
    end;
    ppLinux64:
    begin
      AReport.Add('Componente compilado para: Linux 64 bits');
      AReport.Add('Serão procuradas apenas bibliotecas Linux 64 bits.');
    end;
    ppLinux32:
    begin
      AReport.Add('Componente compilado para: Linux 32 bits');
      AReport.Add('Serão procuradas apenas bibliotecas Linux 32 bits.');
    end;
    ppLinuxARM64:
    begin
      AReport.Add('Componente compilado para: Linux ARM 64 bits');
      AReport.Add('Serão procuradas apenas bibliotecas Linux ARM 64 bits.');
    end;
    ppLinuxARM:
    begin
      AReport.Add('Componente compilado para: Linux ARM 32 bits');
      AReport.Add('Serão procuradas apenas bibliotecas Linux ARM 32 bits.');
    end;
    ppDarwin64:
    begin
      AReport.Add('Componente compilado para: macOS 64 bits');
      AReport.Add('Serão procuradas apenas dylibs macOS 64 bits.');
    end;
  else
    AReport.Add('Componente compilado para: Plataforma Desconhecida');
  end;
  AReport.Add('Lazarus architecture: ' + FLazarusArchitecture);
  AReport.Add('Operating system: ' + {$IFDEF MSWINDOWS}'Windows'{$ELSE}'Linux'{$ENDIF});
  
  if FExecutionMode = pemDLL then
  begin
    AReport.Add('Execution Mode: DLL');
    AReport.Add('Configured DLLPath: ' + FDLLPath);
    AReport.Add('Loaded DLL: ' + FLoadedDLLPath);
  end
  else
  begin
  ...
  begin
    ...
    AReport.Add(Format('[%s] Py_Finalize (or Py_FinalizeEx)', [BoolToStr(Assigned(Py_Finalize) or Assigned(Py_FinalizeEx), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] Py_IsInitialized', [BoolToStr(Assigned(Py_IsInitialized), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] PyRun_SimpleString (or PyRun_SimpleStringFlags)', [BoolToStr(Assigned(PyRun_SimpleString) or Assigned(PyRun_SimpleStringFlags), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] Py_GetVersion', [BoolToStr(Assigned(Py_GetVersion), 'OK', 'FAIL')]));
    AReport.Add('');
    
    AReport.Add('Optional methods:');
    AReport.Add(Format('[%s] PyImport_AddModule', [BoolToStr(Assigned(PyImport_AddModule), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] PyModule_GetDict', [BoolToStr(Assigned(PyModule_GetDict), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] PyDict_GetItemString', [BoolToStr(Assigned(PyDict_GetItemString), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] PyObject_Str', [BoolToStr(Assigned(PyObject_Str), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] PyUnicode_AsUTF8', [BoolToStr(Assigned(PyUnicode_AsUTF8), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] Py_DecRef', [BoolToStr(Assigned(Py_DecRef), 'OK', 'FAIL')]));
    AReport.Add(Format('[%s] Py_IncRef', [BoolToStr(Assigned(Py_IncRef), 'OK', 'FAIL')]));
    AReport.Add('');
  end;
  
  AReport.Add('Execution test:');
  AReport.Add(Format('[%s] ExecString', [BoolToStr(FInitialized, 'OK', 'FAIL')]));
  AReport.Add(Format('[%s] GetVar', [BoolToStr(FInitialized, 'OK', 'FAIL')]));
  AReport.Add(Format('[%s] Eval', [BoolToStr(FInitialized, 'OK', 'FAIL')]));
  AReport.Add('');
...
begin
  ...
  begin
    ...
    IsError := False;
    ErrLines := '';
    while ReadLineFromProcess(Line) do
    begin
...
begin
  ...
  begin
    if not Assigned(PyImport_AddModule) or not Assigned(PyModule_GetDict) or 
